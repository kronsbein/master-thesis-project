...
class ToGraphListTransformer(BaseEstimator, TransformerMixin):
    ...
    def get_values(row: pd.Series, cols):
        ...
        value_array = np.array([
            sum(el, []) if isinstance(el, list) and all([isinstance(l, list)
                                                         for l in el]) else el
    ...
    def transform(self, X, y=None):
        ...
        for _, row in X.iterrows():
            ...
            props: dict = {
                "x_emb": self.get_values(row, self.emb_columns),
                "x_opt": self.get_values(row, self.opt_columns)
            }
            # get addititional properties
            for col in self.extra_columns:
                props[col] = self.get_values(row, [col])
            # extract label
            props["y"] = self.get_values(row, self.target_column)

            # create and add edge_index (describes connectivity of graph)
            self.conn_dict, props["edge_index_emb"] = self.create_edge_index(
                self.conn_dict, props["x_emb"].size(0))
            self.conn_dict, props["edge_index_opt"] = self.create_edge_index(
                self.conn_dict, props["x_opt"].size(0))
...
class WrapperTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,
                 in_column: Union[str, list] = None,
                 out_column: Union[str, list] = None,
                 transformer=None):
        """
        Parameters
        ----------
    ...
    def fit(self, X, y=None):
        ...
        if isinstance(self.transformer, MinMaxScaler) or isinstance(self.transformer,
                                                                    OneHotEncoder):
            ...
            if len(unique_types) == 1:  # not pre-trained / does not matter
                ...
                self.transformer.fit(X.loc[X.type == 1,
                                           self.in_column].values.reshape(-1, len(self.in_column)))
    ...
    def __transform_with_hasher__(self, row: pd.Series):
        ...
        for el in row.to_list():
            ...
            base_encoding: List[Union[int, float]] = self.transformer.transform(
                [el_str]).toarray().reshape(-1).tolist()
            values.append(base_encoding)

        values = [[0] + sub_list for sub_list in values]  # category "hasher"
        return pd.Series(values)

    def __transform_with_one_hot_encoder__(self, row: pd.Series):
        values = self.transformer.transform(row.values.reshape(-1, 1))
        return pd.Series(values.tolist())

    def __transform_with_binarizer__(self, row: pd.Series):
        values: List[List[Union[int, float]]] = [
            self.transformer.transform([e]).flatten().tolist() for e in row.to_list()
        ]
    ...
    def transform(self, X, y=None):
        ...
        if isinstance(self.transformer, HashingVectorizer) and is_subset:
            X[self.out_column] = X.loc[:, self.in_column].apply(self.__transform_with_hasher__,
                                                                axis=1)

        elif isinstance(self.transformer, OneHotEncoder) and is_subset:
            X[self.out_column] = X.loc[:, self.in_column].apply(
                self.__transform_with_one_hot_encoder__, axis=1)

        elif isinstance(self.transformer, BinaryTransformer) and is_subset:
            X[self.out_column] = X.loc[:, self.in_column].apply(self.__transform_with_binarizer__,
                                                                axis=1)

        elif is_subset:
            X[self.out_column] = self.transformer.transform(X[self.in_column].values.reshape(
                -1, len(self.in_column)))

        # just for some verbose action
        if isinstance(self.transformer, MinMaxScaler):
            logging.info(
                f"MinMaxScaler: data_min_={self.transformer.data_min_}, data_max_={self.transformer.data_max_}"
            )

        if isinstance(self.transformer, OneHotEncoder):
            logging.info(
                f"OneHotEncoder: n_features_in_={len(self.transformer.categories_[0])}, categories_={self.transformer.categories_}"
            )
